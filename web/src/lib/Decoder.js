  ...
  return (function () {
    ...
    var Decoder = function (parOptions) {
      ...
      var onPicFun = function ($buffer, width, height) {
        ...
        if (!buffer) {
          buffer = this.pictureBuffers[$buffer] = toU8Array(
            $buffer,
            (width * height * 3) / 2
          );
        }
      ...
      if (this.options.sliceMode) {
        onPicFun = function ($buffer, width, height, $sliceInfo) {
          ...
          if (!sliceInfo) {
            sliceInfo = this.pictureBuffers[$sliceInfo] = toU32Array(
              $sliceInfo,
              18
            );
          }
      ...
      var ModuleCallback = getModule.apply(fakeWindow, [
        function () {},
        onPicFun,
      ]);
